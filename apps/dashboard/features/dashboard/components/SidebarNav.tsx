...
function isActive(pathname: string, href: string) {
  ...
  if (href === "/tours") {
    return pathname === "/tours" || (pathname.startsWith("/tours/") && !pathname.endsWith("/new"));
  }
...
export function SidebarNav() {
  ...
  return (
    <aside className="flex w-full shrink-0 flex-col gap-6 rounded-[28px] border border-line bg-[color:var(--sidebar-bg)] p-5 shadow-[0_12px_28px_rgba(15,23,42,0.04)] lg:sticky lg:top-6 lg:max-w-[296px] lg:min-h-[calc(100vh-3rem)] lg:self-start">
      <div className="space-y-3 rounded-[24px] border border-line bg-[color:var(--sidebar-surface)] p-4 shadow-[inset_0_1px_0_rgba(255,255,255,0.75)]">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-[11px] font-semibold uppercase tracking-[0.24em] text-ink-muted">Tour Show</p>
            <h1 className="text-lg font-semibold text-[color:var(--sidebar-text)]">Admin Workspace</h1>
          </div>
        </div>
      </div>
      ...
      <nav className="space-y-1.5">
        {navItems.map((item) => {
          ...
          return (
            <LoadingLink key={item.href} href={item.href} loadingLabel="Geçiliyor..." className={[
      ...
      <div className="mt-auto rounded-[24px] border border-line bg-[color:var(--sidebar-surface)] p-4 shadow-[inset_0_1px_0_rgba(255,255,255,0.72)]">
        <div className="flex items-center gap-3">
          <span className="flex h-10 w-10 items-center justify-center rounded-2xl border border-line bg-white text-[color:var(--sidebar-text)]"><SettingsRoundedIcon sx={{ fontSize: 20 }} /></span>
          <div>
            <p className="text-sm font-semibold text-[color:var(--sidebar-text)]">Panel Ayarları</p>
            <p className="text-xs leading-5 text-[color:var(--sidebar-muted)]">Tercihler</p>
          </div>
        </div>
      </div>
    </aside>
