    ...
    >
        ...
        <SectionCard title="Hızlı İşlemler" description="">
          <div className="grid gap-3 lg:grid-cols-3">
            <LoadingLink
              href="/tours/new"
              loadingLabel="Form açılıyor..."
              className="rounded-[24px] border border-line bg-[linear-gradient(180deg,#ffffff,var(--brand-soft))] p-5 transition hover:border-brand/25 hover:shadow-[0_18px_32px_rgba(79,70,229,0.12)]"
            >
              <div className="mb-4 flex h-11 w-11 items-center justify-center rounded-[18px] bg-brand-soft text-brand-strong">
            ...
            <LoadingLink
              href="/tours"
              loadingLabel="Liste açılıyor..."
              className="rounded-[24px] border border-line bg-[linear-gradient(180deg,#ffffff,#f8fafc)] p-5 transition hover:border-line-strong hover:shadow-[0_18px_32px_rgba(15,23,42,0.08)]"
            >
              <div className="mb-4 flex h-11 w-11 items-center justify-center rounded-[18px] bg-panel-strong text-ink-soft">
            ...
            <div className="rounded-[24px] border border-line bg-[linear-gradient(180deg,#ffffff,#f9fafb)] p-5">
              <div className="mb-4 flex h-11 w-11 items-center justify-center rounded-[18px] bg-warning-soft text-warning-strong">
        ...
        <SectionCard title="Durum Kartları" description="">
          <div className="space-y-3">
            <div className="rounded-[22px] border border-line bg-panel-subtle p-4">
              <p className="text-sm font-medium text-ink-muted">Yayın</p>
              <p className="mt-2 text-2xl font-semibold tracking-[-0.04em] text-ink">
                {overview.totals.active}/{overview.totals.total}
              </p>
            </div>
            <div className="rounded-[22px] border border-line bg-panel-subtle p-4">
              <p className="text-sm font-medium text-ink-muted">Bekleyen</p>
              <p className="mt-2 text-2xl font-semibold tracking-[-0.04em] text-ink">{overview.attentionTours.length}</p>
            </div>
            <div className="rounded-[22px] border border-line bg-panel-subtle p-4">
              <p className="text-sm font-medium text-ink-muted">Pasif</p>
              <p className="mt-2 text-2xl font-semibold tracking-[-0.04em] text-ink">{overview.totals.passive}</p>
            </div>
          </div>
        </SectionCard>
      </div>
      ...
      <div className="grid gap-6 2xl:grid-cols-[1.15fr_0.85fr]">
        <SectionCard
          ...
          description=""
          action={
            <LoadingLink href="/tours" loadingLabel="Liste açılıyor..." className={buttonClassName({variant: 'ghost', size: 'sm'})}>
              Tüm liste
            </LoadingLink>
          }
        >
            ...
            <div className="divide-y divide-line">
              {overview.recentTours.map(({tour, completion, status}) => (
                <LoadingLink
                  ...
                  className="grid gap-3 px-4 py-4 transition hover:bg-panel-subtle lg:grid-cols-[1.4fr_0.7fr_0.8fr_0.9fr] lg:items-center"
                >
                  <div className="space-y-1">
                    <p className="font-medium text-ink">{tour.title}</p>
                    <p className="text-sm text-ink-muted">/{tour.slug}</p>
                  </div>
                  <div>
                    <StatusBadge status={status} />
                  </div>
                  <div className="flex items-center gap-3">
                    <div className="h-2.5 flex-1 rounded-full bg-panel-strong">
                      <div className="h-full rounded-full bg-brand" style={{width: `${completion}%`}} />
                    </div>
                    <span className="text-sm font-medium text-ink">{completion}%</span>
                  </div>
          ...
          <div className="space-y-3">
            {overview.attentionTours.map(({tour, issues, status}) => (
              <LoadingLink
                ...
                href={`/tours/${tour.slug}/edit`}
                loadingLabel="Tur açılıyor..."
                className="block rounded-[22px] border border-line bg-panel-subtle p-4 transition hover:border-warning-strong/30 hover:bg-white"
              >
                <div className="flex items-start justify-between gap-4">
                  <div className="space-y-2">
                    <div className="flex items-center gap-2">
                      <h3 className="font-medium text-ink">{tour.title}</h3>
                      <StatusBadge status={status} />
                    </div>
                    <div className="flex flex-wrap gap-2">
                      {issues.slice(0, 3).map((issue) => (
                        <span key={issue} className="rounded-full bg-warning-soft px-2.5 py-1 text-xs font-medium text-warning-strong">
                          {issue}
                        </span>
                      ))}
                    </div>
